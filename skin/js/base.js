...
function checkEmail() {
    ...
    if (!CheckMail.test(email.value)) {
        $.ligerDialog.warn("邮箱格式不正确！");
        return false;
    }

    var CheckMobile = /^0?(13[0-9]|15[012356789]|18[0236789]|14[57])[0-9]{8}$/;
    var mobile = document.getElementById("txtUserTel");
    if (!CheckMobile.test(mobile.value)) {
        $.ligerDialog.warn("手机号码不正确！");
        return false;
    }

    var biaoti = document.getElementById("txtTitle");
    if (biaoti.value == "") {
